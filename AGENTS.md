...
| `check-liveness.mjs` / `liveness-core.mjs` | Job posting liveness checker + shared logic (expired signals win over generic Apply text); `--api-only` stops at the free ATS rung, `--no-log` skips the expired log |
| `expired-log.mjs` | Standing record of postings confirmed removed (`data/expired-jobs.md`) — written by scan and liveness, read by scan dedup and `prune-pipeline.mjs`; only definitive expiries, never `uncertain` |
| `prune-pipeline.mjs` | Retires pending pipeline entries the expired log says are dead (`--stale <days>` also retires old-but-live postings) |
